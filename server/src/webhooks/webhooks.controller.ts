...
// ─── Every route in this controller is external-facing (Meta,
// DoubleTick, Evolution call these directly) — none of them can send
// a JWT, so the whole controller is exempt from the global JWT guard.
// Each route instead checks its own secret/token from the provider.
@Public()
@Controller()
export class WebhooksController {
  constructor(
  ...
  async receiveDoubletick(
    ...
    const expected =
      this.envConfig.get<string>('DOUBLETICK_WEBHOOK_SECRET') || '';
    if (expected && secret !== expected) return { error: 'bad webhook secret' };

    let n = 0;
    for (const item of this.ingest.parseDoubletickPayload(payload)) {
      if (await this.ingest.ingestMessage({ ...item, source: 'doubletick' }))
        n++;
    }
    return { ingested: n };
  }
  ...
  async receiveEvolution(
    ...
    const expected =
      this.envConfig.get<string>('EVOLUTION_WEBHOOK_SECRET') || '';
    if (expected && apikey !== expected) return { error: 'bad webhook secret' };
    ...
    let n = 0;
    for (const item of this.ingest.parseEvolutionPayload(payload)) {
      if (await this.ingest.ingestMessage({ ...item, source: 'evolution' }))
        n++;
    }
    return { ingested: n };
  }
  ...
  @Post('webhooks/import/txt')
  async importTxt(@Body() text: string, @Query('groupKey') groupKey?: string) {
    const debug = process.env.DEBUG_VERBOSE_LOGGING === 'true';
    ...
    const parsed = this.parseChatExport(text);
    // console.log('Parsed messages:', parsed.length);
    if (debug) console.log('Parsed messages:', parsed.length);

    for (const { dt, sender, body } of parsed) {
      // console.log('Processing:', sender, '-', body.slice(0, 30));
      if (debug) console.log('Processing:', sender, '-', body.slice(0, 30));
      try {
        const ok = await this.ingest.ingestMessage({
          groupKey: gk,
          senderKey: sender,
  ...
  // Supports both iOS format: [25/07/26, 3:29:00 PM] Sender: message
  // and Android format: 25/07/26, 15:29 - Sender: message
  private parseChatExport(
    text: string,
  ): { dt: Date; sender: string; body: string }[] {
    // const iosRe = /\[(\d{1,2}\/\d{1,2}\/\d{2,4}), (\d{1,2}:\d{2}:\d{2}\s?[AP]M)\] ([^:\[\]]+): /g;
    // const androidRe = /(\d{1,2}\/\d{1,2}\/\d{2,4}), (\d{1,2}:\d{2}(?:\s?[AP]M)?) - ([^:\n]+): /g;
    ...
    if (process.env.DEBUG_VERBOSE_LOGGING === 'true') {
      console.log('CLEAN TEXT START---');
      console.log(JSON.stringify(cleanText));
      console.log('---CLEAN TEXT END');
    }
    ...
    const androidHits = [...cleanText.matchAll(androidRe)];
    const matches =
      iosHits.length >= androidHits.length ? iosHits : androidHits;
    // console.log(
    //   'iOS matches:',
    //   iosHits.length,
    //   '| Android matches:',
    //   androidHits.length,
    // );
    if (process.env.DEBUG_VERBOSE_LOGGING === 'false') {
      // use true for messages and false for not displaying messages
      console.log(
        'iOS matches:',
        iosHits.length,
        '| Android matches:',
  ...
  private parseDate(d: string, t: string): Date {
    ...
    // Try 12-hour with seconds: 25/07/26, 3:29:00 PM
    const m12s = combined.match(
      /^(\d{1,2})\/(\d{1,2})\/(\d{2}), (\d{1,2}):(\d{2}):(\d{2})\s?(AM|PM)$/i,
    );
    if (m12s) {
      const dd = parseInt(m12s[1]);
      const mm = parseInt(m12s[2]);
